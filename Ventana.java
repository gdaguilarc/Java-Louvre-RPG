...
public class Ventana extends JFrame{
	...
	private Heroe heroe;
	private JLabel snombre, pes, stipo, svida, svelocidad, sspeed, sdefensa, sdefence, snivel, sattack ,sname, stype, slife, slevel, spower, sdescription, des, sexp, sxp;
	...
	public void initComponents2(){
		...
		pes.setText("Te has encontrado con un "+louvre.getCasilla(index).getObjeto().getNombre()+" Deseas conservarlo??");
		des.setText("  Un enemigo "+louvre.getCasilla(index).getEnemigo().getNombre()+" salvaje con "+louvre.getCasilla(index).getEnemigo().getVida()+" de vida. Que haras contra el?");
	}
	public void initTurn(){
		try{
			...
			des.setText("  Un enemigo "+louvre.getCasilla2(index).getEnemigoVivo().getNombre()+" salvaje con "+louvre.getCasilla(index).getEnemigo().getVida()+" de vida. Que haras contra el?");
	...
	public class Conservar implements ActionListener{
		public void actionPerformed (ActionEvent e){
			try{
				...
				controlesAtack.add(controlesAtack2);
				controlesAtack.add(fisico);
				des2.setText("Has agregado un "+louvre.getCasilla(index).getObjeto().getNombre()+" a tu mochila, tienes "+Heroe.numeroObjetos+" de objetos.");
				revalidate();
				repaint();
	...
	public class AtaqueNormal implements ActionListener{
		public void actionPerformed (ActionEvent e){
			try{
				...
				getContentPane().removeAll();
				louvre.getHeroe().levelUp(louvre.getHeroe().getXp()+louvre.getHeroe().gainXp(louvre.getCasilla(index).getEnemigo()));
				add(controles);
				add(malla);
				add(info);
