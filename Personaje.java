...
public class Personaje implements Atk, Serializable{
	...
	public void ataque(Personaje personaje)throws MuerteEnemigoException, MuerteHeroeException{
		if(personaje.getNombre().equals("Momia") || personaje.getNombre().equals("Obra de arte") || personaje.getNombre().equals("Escultura") || personaje.getNombre().equals("LaGioconda") || personaje.getNombre().equals("LaGioconda") || personaje.getNombre().equals("LaVenusDeMilo") || personaje.getNombre().equals("ElHamurabi") || personaje.getNombre().equals("LaLibertadGuiandoAlPueblo")){
			if(personaje.getTipo().equals(this.tipo)){
			  personaje.setVida(personaje.getVida()-(int)(0.01*2*((int)(Math.random()*15)+85)*(((0.2*nivel+1)*getAttack()*60/(25*personaje.getDefensa()))+2)));
			  if(personaje.getVida()<=0){	
			    throw new MuerteEnemigoException();
			  }
			}
			else{
			  personaje.setVida(personaje.getVida()-(int)(0.01*.5*((int)(Math.random()*15)+85)*(((0.2*nivel+1)*getAttack()*60/(25*personaje.getDefensa()))+2)));
			  if(personaje.getVida()<=0){
			    throw new MuerteEnemigoException();
			  }
			}
